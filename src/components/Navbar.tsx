      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-0 start-0 w-full h-screen bg-dark-navy z-[60] flex flex-col items-center justify-center gap-8"
          >
            <button 
              type="button"
              aria-label={t('closeMenu')}
              className="absolute top-6 end-6 text-white"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              <X size={32} />
            </button>
            
            <LanguageSwitcher isMobile onSelect={() => setIsMobileMenuOpen(false)} />
